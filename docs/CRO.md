...
- `newsletter_subscribed`: authoritative client conversion when Netlify Forms returns the visitor to `/thank-you.html`
...
The event map is `newsletter_submit` (intent) → Netlify Forms processing → `newsletter_subscribed` (success-page view). Do not treat submit-button clicks as delivered subscribers.

## Lead operations
Growth/Revenue Operations owns the `hello@hoyack.com` newsletter intake. The owner verifies each accepted subscription, records the source and offer tags, and sends a welcome confirmation within one business day. Delivery failures or a missing Netlify Forms activation are escalated to CTO; consent and unsubscribe requests are handled by Growth/Revenue Operations with Privacy support.
